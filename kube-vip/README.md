...
kube-vip manifest pod \
    ...
    --services \
    --arp \
    --leaderElection | tee ~/kubernetes/kube-vip/kube-vip.yaml
```
&nbsp;
## DaemonSet

### Download and apply rbac.yaml
...
```
curl --output "kube-vip-cloud-controller.yaml" https://raw.githubusercontent.com/kube-vip/kube-vip-cloud-provider/main/manifest/kube-vip-cloud-controller.yaml
...
```
kubectl expose deployment nginx-deployment --port=80 --type=LoadBalancer --name=nginx --load-balancer-ip=192.168.0.244
```
